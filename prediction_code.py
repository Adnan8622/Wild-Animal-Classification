...
model = load_model('Best Results final itteration/EfficientNetB0.h5')
...
for i in range(a):
    ...
    print(pred_cls)
    if pred_cls=='gazella':
        cv2.imwrite('Predicted classes/Gazella/gazella'+str(i)+'.jpg', (image*255))
        #cv2.imshow('',image)
        #cv2.waitKey(0)
    if pred_cls=='hartebeest':
        cv2.imwrite('Predicted classes/Hartebeest/hartebeest'+str(i)+'.jpg',(image*255))
        #cv2.imshow('',image)
        #cv2.waitKey(0)
    if pred_cls=='wildbeest':
        cv2.imwrite('Predicted classes/Wildbeest/wildbeest'+str(i)+'.jpg',(image*255))
        #cv2.imshow('',image)
        #cv2.waitKey(0)
    if pred_cls=='elephant':
        cv2.imwrite('Predicted classes/Elephant/elephant'+str(i)+'.jpg',(image*255))
        #cv2.imshow('',image)
        #cv2.waitKey(0)
    if pred_cls=='hippopotamus':
        cv2.imwrite('Predicted classes/Hippopotamus/hippopotamus'+str(i)+'.jpg',(image*255))
        #cv2.imshow('',image)
        #cv2.waitKey(0)
    if pred_cls=='Lion':
        cv2.imwrite('Predicted classes/Lion/Lion'+str(i)+'.jpg',(image*255))
        #cv2.imshow('',image)
        #cv2.waitKey(0)
    if pred_cls=='zebra':
        cv2.imwrite('Predicted classes/Zebra/zebra'+str(i)+'.jpg',(image*255))
        #cv2.imshow('',image)
        #cv2.waitKey(0)
    if lab==1:#Geraffi
        cv2.imwrite('Predicted classes/Giraffe/giraffe'+str(i)+'.jpg',(image*255))
        #cv2.imshow('',image)
        #cv2.waitKey(0)
    if pred_cls=='Buffalo':
        cv2.imwrite('Predicted classes/Buffalo/Buffalo'+str(i)+'.jpg',(image*255))
        #cv2.imshow('',image)
        #cv2.waitKey(0)
    if lab==8:#Warthog
        cv2.imwrite('Predicted classes/Warthog/warthog'+str(i)+'.jpg',(image*255))
        #cv2.imshow('',image)
        #cv2.waitKey(0)
